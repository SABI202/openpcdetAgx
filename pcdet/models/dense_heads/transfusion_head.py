...
class TransFusionHead(nn.Module):
    ...
    def predict(self, inputs):
        ...
        if self.dataset_name == "nuScenes":
            ped_idx = self.class_names.index('pedestrian') if 'pedestrian' in self.class_names else None
            cone_idx = self.class_names.index('traffic_cone') if 'traffic_cone' in self.class_names else None
            if ped_idx is not None:
                local_max[:, ped_idx, ] = F.max_pool2d(heatmap[:, ped_idx], kernel_size=1, stride=1, padding=0)
            if cone_idx is not None:
                local_max[:, cone_idx, ] = F.max_pool2d(heatmap[:, cone_idx], kernel_size=1, stride=1, padding=0)
